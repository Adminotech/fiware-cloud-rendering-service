...
Renderer.prototype.states = {
        1: { state: 'OFFLINE', message: 'Renderer is offline, new Renderers should not be redirected here.' },
        2: { state: 'ONLINE', message: 'Renderer is online and ready to server new Renderers.' },
        3: { state: 'FULL:', message: 'Renderers Renderer limit has been reached, no new Renderers should be redirected here.' }
};

Renderer.prototype.send = function( messageString ){
    this.connection.socket.send( messageString, function(error){ if (error !== undefined) { console.warn( 'Error sending message to Renderer', error );}} );
};

...
Room.prototype.getClient = function(peerId) {
  var c = __(this.clients).find(function(client) {
    return parseInt(client.peerId) === parseInt(peerId);
  });
  return c;
};
...
Room.prototype.addClient = function(message, client) {
  ...
  this.distributeMessage(new Message('Room', 'RoomUserJoined', {
    peerIds: [client.peerId]
  }), client);
  // Send userJoined message about everybody to client;

  client.send(new Message('Room', 'RoomUserJoined', {
    peerIds: __(this.clients).pluck('peerId')
  }).toJSON());
...
Room.prototype.removeClient = function(client) {
  ...
  this.distributeMessage(new Message('Room', 'RoomUserLeft', {
    peerIds: [peerId]
  }));
...
Room.prototype.addRenderer = function(renderer) {
  ...
  if (this.renderer === false) {
    ...
    renderer.send(new Message('Room', 'RoomUserJoined', {
      peerIds: __(this.clients).pluck('peerId')
    }).toJSON());
